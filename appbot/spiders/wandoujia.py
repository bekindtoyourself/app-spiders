...
def get_kw_url(kw):
    base_url = u'http://apps.wandoujia.com/search?key=%s'
...
class Wandoujia(scrapy.Spider):
    name = "wandoujia"
    allowed_damains = ['http://apps.wandoujia.com/']
    ...
    def parse_search_result(self, response):
        ...
        next_page_url = response.css('section.all-apps>ol.box-list>li>.info>a::attr(href)').extract_first()
    ...
    def parse(self, response):
        ...
        first_search_result_css = response.css('#app-title::text').extract_first()
        version_css = response.css('.app-attrs>dd[itemprop="softwareVersion"]::text').extract_first()
