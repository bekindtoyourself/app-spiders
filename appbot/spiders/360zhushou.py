...
def get_kw_url(kw):
    base_url = u'http://zhushou.360.cn/search/index/?kw=%s'
...
class Zhushou(scrapy.Spider):
    name = "360zhushou"
    allowed_damains = ['http://zhushou.360.cn']
    ...
    def parse_search_result(self, response):
        ...
        next_page_url = response.css('div.SeaCon>ul>li:nth-child(1)>dl>dd>h3>a::attr(href)').extract_first()
    ...
    def parse(self, response):
        ...
        # first_search_result = response.css('#app-name>span::text').extract_first().strip()

        # version = re.sub(r'[^0-9.]', '', response.css('#sdesc > div > div > table > tbody > tr:nth-child(2) > td:nth-child(1)::text').extract_first().strip())

        # debug error : 'NoneType' object has no attribute 'strip' 
        first_search_result_css = response.css('#app-name>span::text').extract_first()
        version_css = response.css('#sdesc > div > div > table > tbody > tr:nth-child(2) > td:nth-child(1)::text').extract_first()
